...
def check_if_dailies():
    global m, temp, tempc
    if (m == "dailies" and tempc): temp = temp / tempc
...
for i in range(len(data["data"])):
    if (f > data["data"][i][0]): continue
    while f != data["data"][i][0]:
        f = increase_f(f,0)
    multiplier = 1
    if (len(data["data"][i])>2): multiplier = data["data"][i][2]
...
for ir in range(f,t+1):
    #print(ir)
    if (((ir % 100) > 12) or ((ir % 100) == 0)): continue
    if not(cy): resArr.append([ir,0])
...
if (t == currentMonth and m == 'monthly_results'):
    ...
    if (resArr[len(resArr) - 1][1] > maxValue): maxValue = resArr[len(resArr) - 1][1]
