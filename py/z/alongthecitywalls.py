...
for cat in rows:
    ...
    sql = "SELECT c" + str(cat[0]) + " FROM daily_res WHERE c" + \
        str(cat[0]) + " <> '' order by Date desc limit 1"
    conn = cconn.create_connection(db_file)
    cur = conn.cursor()
    cur.execute(sql)
    row = cur.fetchone()
    ...
    if (len(data['data'])):
        ...
        # print(temp_dict['id'])
        while (length > 0 and data['data'][length - 1]
               [0] // 100 == data['data'][length][0] // 100):
            #print(data['data'][length-1][0] // 100)
            temp += data['data'][length - 1][1] * data['data'][length - 1][2]
            tempc += data['data'][length - 1][2]
            if (data['data'][length - 1][3] > tempm):
                tempm = data['data'][length - 1][3]
            length -= 1
