...
if (m[0]):
    modifier = "asc"
sql = "SELECT Date, c" + str(daily) + " FROM daily_res WHERE c" + str(daily) + " <> '' AND c" + \
    str(daily) + " > 1  order by c" + str(daily) + " " + modifier + " limit 5"
cur = conn.cursor()
cur.execute(sql)
rows = cur.fetchall()
...
op = {}
op['top5'] = rows
sql = "SELECT Date, c" + str(daily) + " FROM daily_res WHERE c" + str(daily) + " <> '' AND c" + str(
    daily) + " > 1 AND Date > " + str(ld // 100 * 100) + " order by c" + str(daily) + " " + modifier + " limit 1"
...
sql = "SELECT Date, c" + str(daily) + " FROM daily_res WHERE c" + str(daily) + " <> '' AND c" + str(
    daily) + " > 1 AND Date > " + str(ld // 10000 * 10000) + " order by c" + str(daily) + " " + modifier + " limit 1"
