...
def create_log_file_for_new_person(pId, mnth):
    newPerson = open(
        "..\\logs\\monthly_results\\res_" +
        str(pId) +
        ".json",
        "w+")
    newPerson.write(json.dumps(
        {"pId": pId, "firstAppearance": mnth, "lastAppearance": mnth, "data": [[mnth, 0]]}))
    newPerson.close()
...
def update_entry(conn):
    sql = "UPDATE top SET '1'='" + str(top3[0]) + "', '2'='" + str(
        top3[1]) + "', '3'='" + str(top3[2]) + "' WHERE Date=" + str(d)
    try:
        remove_points()
        add_points()
        cur = conn.cursor()
...
def add_points():
    for i in range(3):
        sql = "UPDATE people SET points=points + " + \
              str(3 - i) + ", pointslm = pointslm + " + str(3 - i) + " WHERE ID=" + top3[i]
        try:
            cur = conn.cursor()
            cur.execute(sql)
            conn.commit()
...
def remove_points():
    ...
    for i in range(3):
        sql = "UPDATE people SET points=points - " + str(3 - i) + ", pointslm = pointslm - " + str(
            3 - i) + " WHERE ID=" + str(oldTop3[i + 1])
        try:
            cur = conn.cursor()
            cur.execute(sql)
            conn.commit()
...
if (rows):
    ...
    if (top3[0] or top3[1] or top3[2]):
        new_entry(conn, (d, str(top3[0]), str(top3[1]), str(top3[2]),))
